...
area-decreasing operator accepts almost every move---$96\%$ on the same test
body---and still fails, because the moves it accepts are overwhelmingly
area-\emph{neutral}. Roughly four in five carry $\Delta A = 0$, cost nothing,
and redistribute nickel without advancing the instability. Applied in batches
they raise the area outright, since per-move guarantees do not compose across a
batch ranked on a stale neighbour field; applied one at a time on real
electrodes they lower the area but inflate triple-phase-boundary density by a
factor of four to five. Neither rule reaches Rayleigh break-up.
...
destroying it, and neither budget an operator is usually given prevents this: a
swap conserving volume exactly and reducing surface area monotonically still
inflates TPB three- to fivefold on real electrodes. Qualification gates demanding perfect pristine connectivity are
...
fail at \SIrange{0.5}{3}{\percent} of throats through small non-planar cuts.
...
surround such an operator interact with each other: the connectivity metric,
the qualification gate on the test structure, and the validity criterion on
the operator itself. We did not adopt the established formulation, and this
...
stochastic surface erosion: at each round, every nickel voxel on the phase boundary is
...
after the fact. First, application is \emph{sequential}: one move, neighbour
field recomputed, repeat. Equation~\eqref{eq:dA} is exact for a single swap
...
buckets indexed by that count. Second, ties are broken \emph{uniformly at
random} among moves of identical $\Delta A$, from a seeded generator. This is
part of the rule, not an implementation detail: area-neutral moves dominate,
...
randomised policy reaches $-0.00955$, a factor of 40
(\texttt{o7\_tiebreak\_sensitivity.py}).

Counting convention. The neighbour count $\nb$ in Eq.~\eqref{eq:dA} excludes
the voxel itself. This is worth stating because the standard 6-connectivity
structuring element in common image-processing libraries includes the centre,
...
The unit those correlations are computed over is the individual bisection, not
the anode: three anodes $\times$ three regions $\times$ three damage seeds
$\times$ two thresholds, i.e.\ 54 rows in
\texttt{c1real\_results.csv}, 27 per threshold. This is worth stating
explicitly, because a rank coefficient over three points could only take the
values $\{0,\pm0.5,\pm1\}$ and a threshold of $0.6$ would then be equivalent to
...
\begin{table*}[tb]
  ...
  specimens. No dispersion is quoted because none exists to quote: each entry
  is a single number computed once over an entire stack, not a mean over
...
nulls are explained by the minimum-cut measurement: an operator confined to
...
so after pruning the surviving component \emph{is} the spanning one and
$\Pspan$ equals unity identically. The operator rewrites the denominator of
the metric used to judge it. This is the third failure mode and the one that
...
The behaviour is identical in two independently implemented operators: on real
...
14.8, 12.3 and 7.4 at the first damage round before it collapses to
\SIrange{4.8}{10.2}{\percent} of pristine by the eighth (Fig.~\ref{fig:tpb}). The mechanism is
discretisation: removing single voxels
from a surface roughens it at the voxel scale, and each newly exposed
nickel/pore facet that happens to touch zirconia adds triple line.
...
single-voxel swap that conserves volume exactly \emph{and} decreases specific
...
about \SI{80}{\percent} of accepted moves carry $\Delta A = 0$ exactly---on the
fine anode, $208\,180$ of $258\,870$---and are therefore unpriced by an area
criterion (Fig.~\ref{fig:gatetpb}b). An area budget does not constrain the
...
ceramic rather than by more nickel. On the fine anode
\SI{92.8}{\percent} of removed voxels are adjacent to YSZ: the operator strips
nickel off the ceramic. Added material, by contrast, lands almost anywhere,
only \SI{3.2}{\percent} of it beside YSZ, and arrives as
\num{34753} fragments of median size two voxels.
...
reproduce the whole artifact, $\times 5.07$ against $\times 5.03$ for the full
set; every other move, which is \SI{97}{\percent} of the added material, moves
TPB by four percent. The mechanism is therefore specific rather than diffuse:
the area criterion rewards precisely the moves that pit the nickel--zirconia
contact patch, and the triple line lives on that patch's perimeter. The gate
...
\begin{figure*}[tb]
  ...
  and the horizontal bar is the class mean. Fine is the \emph{last} anode to
  fail. (c) Transition
...
$+0.0145$ across the three anodes and therefore \emph{increases} in the coarse
...
still fails the gate---but not by refusing to move. Applied in batches of
$k = 0.03\,N_{\mathrm{surf}}$ per round, as implemented, the specific surface
area \emph{rises} at every intensity, from $0.45052$ pristine to $0.46365$ at
...
variant---one move, neighbour field recomputed, repeat---does lower the area,
and on all three real electrodes it passes the strict-decrease gate: specific
...
The rule is permissive rather than selective on real data: essentially every
...
regions of interest, triple-phase-boundary density \emph{rises} by factors of
...
carry $\Delta A = 0$ exactly: on the fine electrode, $208\,180$ of $258\,870$.
...
the contact with zirconia; \SI{92.8}{\percent} of removed voxels sit against
...
variant, at any intensity, seed or move budget tested, did the neck thin: the
...
its own validity criterion. The two cannot hold together. This is the sixth
and most general of the failure modes, and unlike the other five it is a
statement about what an operator class can represent rather than about how a
particular implementation goes wrong.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=\linewidth]{fig7_gate_vs_tpb.pdf}
  ...
  triple-phase-boundary density rises by a factor of $5.03$. (b) Why the gate
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{fig8_counterfactual.pdf}
...
facts of geometry rather than from this dumbbell: a sphere minimises area at
...
\begin{table*}[tb]
  \centering
  \caption{The six failure modes, in the order the work encountered them.
  Every one arose from a combination of individually defensible choices, and
  every one was invisible on the synthetic platform for the reason given in
  the last column.}
  \label{tab:modes}
  \small
  \begin{tabular}{@{}c p{0.26\textwidth} p{0.31\textwidth} p{0.31\textwidth}@{}}
    ...
    \midrule
    1 & lattice minimum cut is a plane & cut is exactly $6^2$, $5^2$, $4^2$
      edges, zero variance & regular lattices were never asked to fail
      locally \\
    2 & gate demands perfect pristine connectivity & real electrodes carry
      \SIrange{1.2}{11.2}{\percent} isolated Ni & synthetic structures were
      built fully connected \\
    3 & pruning fixes $\Pspan$ at unity & $\Pspan$ jumps to 1.0000 at the
      first damage round & pristine $\Pspan$ was already 1.0000 by gate \\
    4 & voxel moves manufacture TPB & $7$--$15\times$ under erosion;
    ...
    5 & curvature rank $\neq$ area change & $\Sspec$ rises at $n=1$ under
      both stencils & no explicit area budget was ever imposed \\
    6 & area monotonicity excludes break-up & $80\%$ of accepted moves are
...
diffusion as the exchange of a pore site with a neighbouring solid site: a
...
them as a validity criterion rather than as a choice of dynamics. That is why
the impossibility is narrow, and it is also why it is easy to walk into: the
gate looks like rigour and functions like a constraint on the physics. Had we
imposed the same gate on the model of \cite{zhou2023}, it would have failed
too.
...
should have been the starting point. What this paper adds is not the discovery
that a better method exists but the measurement of what the worse one costs.
...
specimen variation and no paired quantity is available. That is a property of
this dataset rather than of the problem: ex-situ tomography of a single
...
evaluated throughout at 6-connectivity. That is the conservative convention,
but it ignores face-diagonal contacts, and the effect is not neutral across the
series: the finest anode has the most surface per unit volume and therefore the
most opportunity for such contacts, so its pristine connectivity is the most
likely to be understated. A 26-connectivity sensitivity check was not
performed. Fourth, regions are cut with free, non-periodic boundaries, which
...
a surviving candidate: nothing here eliminates its competitors, and no run in
...
withdrawn: corrected, it accepts almost every move. The impossibility above is
untouched, because it turns on which moves are \emph{permitted}, not on how
...
not sufficient: the finite-temperature runs permit area-increasing moves and
...
restricted to monotone area reduction. That is a necessary condition on the
method, not a recipe for the mechanism, and the anticorrelation itself remains
...
\item Enforcing monotone surface-area reduction as a validity criterion
  ...
  area-neutral plateau: roughly $80\%$ of accepted moves carry
...
\item Under the uniform surface-erosion operator tested here, neither pristine
  ...
  degradation in general: a rule with a different spatial bias could rank them
...
\item Real nickel networks fail at \SIrange{0.5}{3}{\percent} of throats
  ...
  must carry comparable positional disorder: coordination standard deviation
