...
\begin{figure}[tb]
  ...
  only in where the cut is placed. The fine anode is the first to fail at none
  of the nine thresholds; the measurement says it should be first at all of
  them. The convergence below 0.50 is the discontinuous collapse of the
...
supports: the fine anode retains nickel percolation worst, and therefore should
...
first to fail at \emph{none} of them (Fig.~\ref{fig:threshold}). The identity of
the anode that does fail first changes with the threshold---coarse above 0.60,
medium at and below 0.50---which is a further symptom of the unresolved pair,
...
\begin{table*}[tb]
  ...
  work encountered them. Modes 1, 2, 3 and 5 are implementation faults: each
  arises from a combination of individually defensible choices and each is
  removable by choosing differently. Modes 4 and 6 are not. They are properties
