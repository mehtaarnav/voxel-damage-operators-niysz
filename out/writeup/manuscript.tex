...
and pore meet and where the electrochemical reaction proceeds, is consumed as
the nickel phase reorganises \cite{monaco2019}. All three are geometric
...
experimentally acquired anode functional layers \cite{chen2011}, through work
directed explicitly at validating such a model \cite{trini2021}, to
...
operator interact with each other: the connectivity metric, the
qualification gate on the test structure, and the validity criterion on the
operator itself.
...
nickel/pore facet that happens to touch zirconia adds triple line. An earlier
volume-conserving redistribution operator, run on synthetic structures, showed
the same signature more mildly: at exactly conserved volume it raised nickel
surface area by \SIrange{1.3}{6.0}{\percent} and roughly doubled TPB, with
per-class ratios of 2.02, 2.10 and 2.26. Roughening is not particular to
removal.
...
a monotonicity gate forbids. Any single-voxel-swap coarsening study that both
...
\begin{table*}[tb]
  ...
  \begin{tabular}{@{}c p{0.26\textwidth} p{0.31\textwidth} p{0.31\textwidth}@{}}
    ...
    4 & erosion manufactures TPB & TPB rises $7$--$15\times$ before
      collapsing & platform TPB was already $\sim8\times$ inflated \\
    5 & curvature rank $\neq$ area change & $\Sspec$ rises at $n=1$ under
      both stencils & no explicit area budget was ever imposed \\
    6 & area monotonicity excludes break-up & greedy rule accepts zero moves;
      Eq.~\eqref{eq:dA} & the operator was never asked to thin a neck \\
    \bottomrule
  \end{tabular}
\end{table*}
...
and validated against measured microstructure \cite{zhou2023}, on the same
lattice discretisation that carries our argument. The distinction that matters
is not continuum against discrete but the acceptance rule: any scheme that
accepts an energy-raising move with finite probability can cross the barrier
of Eq.~\eqref{eq:dA}, and any scheme that refuses one cannot. A monotone
area-reduction gate is a refusal, imposed as a validity criterion rather than
chosen as a dynamics. That is why the impossibility is narrow, and it is also
why it is easy to walk into: the gate looks like rigour and functions like a
constraint on the physics.
