...
\author[a]{Author One\corref{cor}}
\ead{author.one@example.org}
\author[a]{Author Two}
\address[a]{Affiliation, City, Country}
\cortext[cor]{Corresponding author.}

\begin{abstract}
Applying a voxel-scale damage operator to a segmented tomogram is a standard
way to simulate microstructural degradation in solid oxide cell electrodes.
We asked whether any operator of this class reproduces a sharp measured
signature in three FIB-SEM reconstructed Ni--YSZ anodes spanning a coarseness
series: the finest anode retains nickel percolation worst (0.680 against 0.855
and 0.947) yet retains triple-phase boundary best. None does. The reasons are
methodological, and they are the useful part of the result.

The most general is an impossibility. Requiring an operator to reduce specific
surface area monotonically is the natural way to validate a coarsening rule,
and it excludes Rayleigh-type neck break-up, the mechanism such rules exist to
represent. On a 6-connected lattice a single-voxel swap changes exposed area
by exactly $2[\nb(a)-\nb(b)]$, so the criterion is a statement about
neighbour counts. A curvature-ranked operator thins a model neck from 63
voxels to zero but violates the criterion at its first step; a greedy
area-decreasing operator never violates it and accepts no move at all, because
a sphere--neck--sphere body is already a local minimum under single-voxel
displacement. Only finite-temperature acceptance bridges the two, and a
monotonicity gate forbids exactly that.

Five further failure modes are measured. Retaining the largest connected
component drives the spanning fraction to unity identically, so the operator
rewrites the denominator of the metric used to judge it. Voxel-scale erosion
multiplies triple-phase boundary density seven- to fifteen-fold before
destroying it. Qualification gates demanding perfect pristine connectivity are
unrepresentative of real electrodes, which carry \SIrange{1.2}{11.2}{\percent}
disconnected nickel. A jittered regular lattice has a minimum cut of exactly
one full cross-section with zero seed-to-seed variance, whereas real networks
fail at \SIrange{1}{2}{\percent} of throats through small non-planar cuts.
Curvature-ranked moves do not deliver the area reduction they are assumed to
deliver. Each was invisible on synthetic test structures and each would
silently corrupt a published study of this kind.
\end{abstract}
...
\cite{monaco2019}. All three are geometric processes acting on a
microstructure that FIB-SEM and X-ray nano-tomography now resolve routinely,
which makes a particular computational strategy very attractive: take a
segmented tomogram, apply a damage operator to the label field, and track how
connectivity and TPB evolve.
...
explicit validation studies against ex-situ imaging of the same specimen
\cite{trini2021}, to quantitative simulations initialised on ptychographic
...
applied on the fabrication side, where a kinetic Monte Carlo model of
NiO--YSZ sintering was calibrated and validated against measured microstructure
\cite{zhou2023}. What is comparatively thin is a systematic account of what
happens when a discrete, voxel-level operator is used for the degradation
problem specifically, and of how the conventions that surround such an
operator---the connectivity metric, the qualification gate on the test
structure, the validity criterion on the operator itself---interact with each
other.
...
signature in three Ni--YSZ anodes, and we did not find one. Two principal
geometry-based operator classes were tested to a conclusion and neither
reproduced the ordering; two further routes closed before a mechanism test was
reached. Along the way six specific ways in which this class of simulation
...
separable: the failure modes were found by running the programme that failed,
and they are reported in the order in which the work forced us to confront
them.
...
the finest microstructure retains nickel percolation \emph{worst} while
retaining TPB \emph{best} (Fig.~\ref{fig:signature}, Table~\ref{tab:target}).
A coarsening argument predicts the first half: by Herring's scaling laws the
...
Medium versus coarse is not resolved on the nickel side: the ordering flips
...
---\emph{fine retains nickel percolation worst} and \emph{fine retains TPB
best}---are defensible, and those are the only claims we test against.
...
paired before-and-after quantity is computed anywhere in this work. This is
the principal reason all comparisons here are ordinal.
...
Two operator families were run to a conclusion. The first is stochastic
surface erosion: at each round, every nickel voxel on the phase boundary is
...
is exactly preserved. Two move-selection rules were implemented, both
pre-registered: ranking candidate sites by a curvature proxy, and greedy
acceptance of a move only when it does not increase exposed area.
...
is what makes the target interesting: whatever mechanism destroys the nickel
...
Work began on a synthetic three-phase platform: nickel spheres on a jittered
...
mechanism was pristine-state loading: the class calibrated to start nearest
...
(Fig.~\ref{fig:mincut}). A jittered regular lattice has no locally weak plane:
every cross-section is statistically identical, so the minimum cut is a plane
...
targeted cut breaks the network at \SIrange{5.8}{7.1}{\percent} of throats,
a factor of about eight apart. The critical edges overlap the lower-quartile
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{fig4_mincut.pdf}
  ...
  interest cut at \SIrange{1}{2}{\percent} of throats with genuine
  region-to-region spread. Data: \texttt{audit\_ni\_vulnerability.csv},
  \texttt{c1real\_results.csv}.}
  \label{fig:mincut}
\end{figure}

The same audit measured the real networks for comparison. Real nickel networks
are disordered in a way the lattice is not: connected-pair-distance
...
plane. This is the first of the failure modes, and it is a statement about
what a synthetic platform can be asked to do. A regular lattice cannot fail
...
The second failure mode surfaced only when the operators were moved to real
...
Damage operators conventionally retain only the largest connected component,
on the physical grounds that isolated nickel is electrically dead. The two
conventions are individually reasonable and jointly circular. Pruning deletes
exactly the non-spanning voxels, so after pruning the surviving component
\emph{is} the spanning one and $\Pspan$ equals unity identically. The operator
rewrites the denominator of the metric used to judge it. This is the third
failure mode and the one that cost us the most time.

On real regions, pristine $\Pspan$ of 0.9821, 0.9713 and 0.8878 became exactly
1.0000 at the first damage step, in two independently implemented operators
(Fig.~\ref{fig:metric}a). Our first diagnosis attributed this to an oxidative
dilation step, on the reasoning that dilating nickel into pore would bridge
the disconnected fraction. That diagnosis was wrong. Removing the dilation
step changed almost nothing---the fine anode's TPB inflation moved from
15.24 to 14.80 and $\Pspan$ remained exactly 1.0000---because the cause is the
pruning step, not the dilation.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=\textwidth]{fig2_metric.pdf}
...
fraction of \emph{pristine} nickel gives a quantity that does not move when
...
\subsection{Voxel-scale erosion manufactures TPB before destroying it}

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{fig3_tpb.pdf}
...
The fourth failure mode is in the same runs. TPB density is multiplied by
14.8, 12.3 and 7.4 at the first damage round before it collapses to a few
percent of pristine by the eighth (Fig.~\ref{fig:tpb}). The mechanism is
discretisation: removing single voxels
from a surface roughens it at the voxel scale, and each newly exposed
nickel/pore facet that happens to touch zirconia adds triple line. The
volume-conserving operator described below showed the same signature more
mildly, doubling TPB while increasing nickel surface area by
\SI{3.3}{\percent}.

That voxelised TPB estimates carry a resolution-dependent systematic bias is
established \cite{jorgensen2014,shimura2016}. What is different here is that
the bias is not a fixed offset but a large, non-monotone excursion driven by
the operator itself, so it cannot be cancelled by taking ratios to the
...
is worth stating as a routine check: it costs one line and it disqualified
...
\begin{figure*}[tb]
  ...
  regions and three damage seeds per anode; bars are class means, points are
  individual bisections. Fine is the \emph{last} anode to fail. (c) Transition
...
means of 9.72 (fine), 9.06 (medium) and 9.39 (coarse) rounds; at a threshold
of 0.10 the values are 9.72, 9.28 and 9.39. The pre-registered criterion
required fine to be lower than both others by at least one round. Fine is
higher at both thresholds, and every separation is also below one round, so
the result is simultaneously wrong in sign and unresolved in magnitude
(Fig.~\ref{fig:reversal}a,b). Against the measurement this is a complete
reversal: real retention runs fine $<$ medium $<$ coarse from worst to best,
while the simulation gives medium $<$ coarse $<$ fine.
...
fragile of the three: minimum cut fractions are
\numrange{0.0141}{0.0196} (fine), \numrange{0.0221}{0.0302} (medium) and
\numrange{0.0055}{0.0067} (coarse). Coarse is also the best retainer in
service, at 0.947. Pristine topological fragility runs three to five times
against the outcome it is supposed to predict. Taken with the absent surface
area correlation, the sharpest statement this dataset supports about the
measured ordering is a statement about what does not control it: neither the
...
where the volume loss at the transition was recorded. Fine loses
\SI{74.4}{\percent} of its nickel before the network breaks, medium
\SI{70.5}{\percent} and coarse \SI{62.8}{\percent}. Fine does erode faster,
and its \num{1.26}-fold specific-surface-area advantage is real, but its
...
Surface erosion removes volume. Real coarsening does not: it conserves nickel
and reduces surface area. A removal-only operator also cannot reproduce the
measured change in nickel volume fraction, which is $-0.1006$, $-0.0165$ and
$+0.0146$ across the three anodes and therefore \emph{increases} in the coarse
...
\begin{figure*}[tb]
  \centering
  \includegraphics[width=\textwidth]{fig6_area_barrier.pdf}
  ...
  greedy operator does nothing. (c) Surface area. The curvature-ranked
  operator enters the region forbidden by its own validity criterion at
  $n=1$, at 0.45195 against a pristine 0.45052, before falling below pristine
  at $n=3$. The greedy operator never leaves the pristine line because it
  accepts no move at all. Data: \texttt{o5v2\_area\_barrier.csv}.}
  \label{fig:barrier}
\end{figure*}

A coarsening operator ought to reduce specific surface area, so requiring it
to do so monotonically is the natural validity gate, and we pre-registered
exactly that gate. Two move-selection rules were implemented against it and
...
(Fig.~\ref{fig:barrier}b). This is genuine Rayleigh-type behaviour, and it
fails the gate, because at the first round the surface area \emph{rises}, to
...
not the area change itself: on a discrete lattice the added voxel exposes new
...
area falls below pristine. This is a fifth failure mode in its own right, and
it is the one that Eq.~\eqref{eq:dA} was derived to repair.

The second rule uses Eq.~\eqref{eq:dA} directly, accepting a swap only when
$\Delta A \leq 0$. It cannot violate the gate by construction, and it accepts
zero moves. Not a small number---zero, at every intensity tested, so that the
structure is returned bit-identical and the gate fails on the requirement of a
\emph{strict} decrease. The reason is not an implementation fault. In a
sphere--cylinder--sphere body the least convex surface voxel still has more
nickel neighbours than the most concave available pore site,
$\nb_{a,\min} > \nb_{b,\max}$, so every candidate pair is rejected at the
first comparison. The body is already a local minimum with respect to
single-voxel moves. Spheres minimise area at fixed volume and a straight
cylinder is stable against any individual voxel displacement.

That is the whole of the argument, and its consequence is a structural one.
...
the other prices it and refuses to cross, and does nothing. The only bridge is
finite-temperature acceptance of area-increasing moves, which is exactly what
a monotonicity gate forbids. Any voxel-based coarsening study that both
...
its own validity criterion. The two cannot hold together.
...
stopped at this point, as the pre-registration required, and the sixth failure
mode is the stopping condition itself. The agglomeration hypothesis is not
falsified by any of this. It is untested, because the operator class available
to us cannot represent it under a gate we still believe is the right gate for
that operator class.
...
untestable outcome is information, and because the alternative---redefining
the criterion until a number emerges---is the failure mode this paper is
about.

The first is redistribution: nickel migrating from the electrolyte-adjacent
...
concerns: gradients are 0.0042 and 0.0240 for fine in its two states, 0.0059
and 0.0552 for medium, and 0.1035 and 0.1157 for coarse. Nickel content does
vary strongly with depth in every stack---coarse post-redox spans 0.045 to
...
\SI{0.65}{\percent} we froze---which establishes that the strain scale was not
the error even though it does not rescue the run.

%======================================================================
\section{Discussion}
%======================================================================

The six failure modes share a structure worth naming. In each case a modelling
...
The prescription that follows is not a general appeal to rigour but something
narrower and cheaper. Validation criteria and mechanisms should be checked
...
by a test structure that was merely \emph{not} fully connected, and a fourth
...
cannot. This is not a hypothetical alternative for this material: phase-field
...
either. A kinetic Monte Carlo model of NiO--YSZ sintering has been calibrated
and validated against measured microstructure \cite{zhou2023}, and the reason
it is not subject to our argument is instructive: it is a Monte Carlo method
with finite-temperature acceptance, so it can cross an energy barrier that a
greedy rule cannot. The barrier described here is a property of the acceptance
rule, not of the lattice.
...
We tested two geometry-based operator classes to a conclusion and neither
reproduces the measured ordering. We have not shown that no geometric
mechanism can. We have specifically not shown that agglomeration is not the
...
specimen variation and no paired quantity is available. The two $\Rni$
...
perimeter where TPB lives. That remains the most plausible candidate, and
testing it needs a formulation that can cross the barrier of
Eq.~\eqref{eq:dA}.
...
\item Enforcing monotone surface-area reduction as a validity criterion
  ...
  the criterion reduces to $\nb(a)\leq\nb(b)$, and a sphere--neck--sphere body
  is already a local minimum under single-voxel swaps, so a greedy
  area-decreasing operator accepts no move. The criterion and the mechanism
  cannot both be honoured by a single-swap operator.
\item Five further failure modes were measured, each invisible on synthetic
  ...
  identically unity; voxel-scale erosion multiplies TPB density by 7 to 15
  before destroying it; qualification gates demanding perfect pristine
  ...
  real networks cut at \SIrange{1}{2}{\percent} of throats; and
...
\item Stochastic surface erosion applied to real microstructures with a
  pruning-invariant metric reverses the measured degradation ordering: the
  fine anode is the last to lose percolation rather than the first.
\item Neither pristine topology nor specific nickel surface area predicts the
  measured ordering. The coarse anode is three to five times the most
  topologically fragile of the three and is the best retainer in service.
\item Real nickel networks fail at \SIrange{1}{2}{\percent} of throats through
  small, region-variable, non-planar cuts. A synthetic generator intended to
  reproduce this must reach mean coordination \numrange{3.4}{3.9} with
  standard deviation \numrange{2.0}{2.4} and a connected-pair-distance
  coefficient of variation near 0.41.
\end{enumerate}
...
project repository \texttt{connectivity\_margin}. Every number in this paper
...
\texttt{step2\_O3\_main.csv}, \texttt{step2\_O3\_control.csv}; the
volume-conserving gate to \texttt{o5v2\_area\_barrier.csv}, transcribed by
\texttt{scripts/project2/o5v2\_transcribe.py} from the two committed run
reports; the redistribution and ceramic-fracture branches to
\texttt{h1\_depth\_profiles.csv}, \texttt{h1\_layer\_summary.csv} and
\texttt{csfm\_results.csv}. Figures regenerate with
\texttt{python scripts/project2/make\_figures.py}.
...
earlier results during the work---a TPB estimator that over-counted fourfold
...
point, and an internally contradictory plan specifying both Metropolis
acceptance and strict area monotonicity---are logged in the run reports at the
point where each was found.
\endgroup
...
\bibitem{xu2014}
X.~Xu, J.~Wang, Z.~Zhou, T.~M. Garoni, Y.~Deng,
Geometric structure of percolation clusters,
Physical Review E 89 (2014) 012120.
